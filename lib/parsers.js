...
function _parseRoutes(routes) {
    ...
    _.forEach(routes, function (controller_action, method_path) {
        ...
        if (_.isFunction(controller_action)) {
            // FIXME figure out what the output should be
            return
        }
        else if (controller_action.fn && _.isFunction(controller_action.fn)) {
            // FIXME figure out what the output should be
            return
        }
        else if (controller_action.response) {
            // FIXME figure out what the output should be
            return
        }
        else if (_.isObject(controller_action)) { // is _.isPlainObject more suitable?
        ...
        if (Object.keys(property).indexOf('http_method') >= 0 && property.http_method !== 'post' && property.http_method !== 'put') {
